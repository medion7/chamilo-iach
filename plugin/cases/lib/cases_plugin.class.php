...
*/
class CasesPlugin extends Plugin
{
 ...
 //                array('name' => 'big_blue_button_welcome_message',  'type' => 'text'),
  ...
  // );
    ...
    function install() {
        ...
        $sql = "CREATE TABLE IF NOT EXISTS `c_cases` (
				`id` int(11) NOT NULL,
				`cid` int(11) NOT NULL,
    ...
    function uninstall() {
        ...
        $sql = "DELETE FROM $t_settings WHERE sub_key = 'cases'";
        Database::query($sql);

        //hack to get rid of Database::query warning (please add c_id...)
        $sql = "DELETE FROM $t_tool WHERE name = 'cases' AND c_id = c_id";
        Database::query($sql);
