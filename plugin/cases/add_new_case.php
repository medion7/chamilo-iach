...
echo '</div>';
	...
	$sql_userCases = "SELECT `id` FROM `c_cases` WHERE `user_id` = ".$user_id." AND `cid` = ".$course_id." AND `session_id` = ".$session_id;
	...
	if ($form->validate()) {
		...
		if(Database::num_rows($res_userCases) == 0 && !isset($_GET['terms']) && empty($_POST)){
			echo get_lang('Terms');
