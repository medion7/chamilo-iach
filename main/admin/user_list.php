...
function modify_filter($user_id, $url_params, $row) {
	...
	if (api_is_platform_admin(true)) {    
		...
		if (!$user_is_anonymous && api_global_admin_can_edit_admin($user_id, null, true)) {
			//Added by Medion7
			$result .= '<a href="videostats/videostats_user.php?student='.$user_id.'">'.Display::return_icon('videostats.png', get_lang('VideoStats')).'</a>&nbsp;&nbsp;';
            $result .= '<a href="user_edit.php?user_id='.$user_id.'">'.Display::return_icon('edit.png', get_lang('Edit'), array(), ICON_SIZE_SMALL).'</a>&nbsp;';
		} else {
			//Added by Medion7
			$result .= Display::return_icon('videostats_na.gif', get_lang('VideoStats')).'&nbsp;&nbsp;';
...
$table->set_header(10, get_lang('Action'), false, 'width="250px"');
