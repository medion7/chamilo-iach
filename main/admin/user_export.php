...
$form->addElement('radio', 'file_type', null, 'Penny','penny');
...
if ($form->validate()) {
	...
	if ($export['addcsvheader']=='1' AND $export['file_type']=='csv') {
		if($_configuration['password_encryption']!='none') {
			$data[] = array('UserId', 'LastName', 'FirstName', 'Email', 'UserName', 'AuthSource', 'Status', 'OfficialCode', 'PhoneNumber', 'Active', 'RegistrationDate');
		} else {
			$data[] = array('UserId', 'LastName', 'FirstName', 'Email', 'UserName','Password',  'AuthSource', 'Status', 'OfficialCode', 'PhoneNumber', 'Active', 'RegistrationDate');
		}
	...
	//Add Penny's export
	$penny = array();
	if ($export['addcsvheader']=='1' AND $export['file_type']=='penny') {
		$penny[0] = array('UserId', 'Active', 'RegistrationDate', 'Salutation', 'FirstName', 'Lastname', 'Email', 'Country', 'Other_titles', 'Hearabout', 'UserName', 'PhoneNumber', 'Address', 'Postcode', 'City_state');
		$i = 1;
	}else{
		$i = 0;
	}
	foreach($data as $my_fields){
		$penny[$i] = array('UserId' => $my_fields['UserId'], 'Active' => $my_fields['Active'], 'RegistrationDate' => $my_fields['RegistrationDate'], 'Salutation' => $my_fields['salutation'], 'FirstName' => $my_fields['FirstName'], 'Lastname' => $my_fields['LastName'], 'Email' => $my_fields['Email'], 'Country' => $my_fields['country'], 'Other_titles' => $my_fields['other_titles'], 'Hearabout' => $my_fields['hearabout'], 'UserName' => $my_fields['UserName'], 'PhoneNumber' => $my_fields['PhoneNumber'], 'Address' => $my_fields['address'], 'Postcode' => $my_fields['postcode'], 'City_state' => $my_fields['city_state']);
		$i++;
	}
	...
	switch($file_type) {
		...
		case 'csv':
			...
			break;
		case 'penny':
			Export::export_table_csv($penny,$filename);
			exit;
			break;
	}
