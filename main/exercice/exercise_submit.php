...
if (!empty($error)) {
    ...
    echo '<script>
          		...
          		$("#save_for_now_"+question_id).html("'.addslashes(Display::return_icon('loading1.gif')).'");
                    $.ajax({
                        ...
                        success: function(return_value) {
                        	if (return_value == "ok") {
                        		$("#save_for_now_"+question_id).html("'.addslashes(Display::return_icon('save.png', get_lang('Saved'), array(), ICON_SIZE_SMALL)).'");
                        	}else if (return_value == "partial") {
								window.location = "exercice.php?'.api_get_cidreq().'";
			...
			function partial_submit() {
   				save_now_all("partial");
                return false;
            }
			
			function partial_submit_opp(question_id) {
				url = "exercice.php?'.api_get_cidreq().'";
   				save_now(question_id, url);
                return false;
            }
		</script>';
        ...
        echo '<div id="question_div_'.$questionId.'" class="main_question '.$remind_highlight.'" >';
            ...
            switch ($objExercise->type) {
                case ONE_PER_PAGE:
					...
					$exercise_actions .= '&nbsp;'.Display::url(get_lang('SaveTest'), 'javascript://', array('onclick'=>'partial_submit_opp(\''.$questionId.'\');', 'class'=>'btn btn-warning'));
                    break;
    ...
    if ($objExercise->type == ALL_ON_ONE_PAGE) {
    	$exercise_actions =  $objExercise->show_button($questionId, $current_question);
    	$exercise_actions .= '&nbsp;'.Display::url(get_lang('SaveTest'), 'javascript://', array('onclick'=>'partial_submit();', 'class'=>'btn btn-warning'));
