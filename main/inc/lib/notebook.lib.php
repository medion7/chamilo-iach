...
class NotebookManager {
    ...
    static function display_notes() {
        ...
        while ($row = Database::fetch_array($result)) {
			...
			echo '<a href="' . api_get_self() . '?action=exportnote&amp;notebook_id='.$row['notebook_id'].'">' . Display::return_icon('pdf.png', get_lang('Export'), '', ICON_SIZE_SMALL) . '</a>';
	...
	function export_notes_to_pdf($desc, $title){
			$header = '<h2>'.$title.'<hr></h2>';
			$html = '<br/><h3>My notes</h3><hr>';
			$html .= '<div>'.$desc.'</div>';
			$html .= '</body></html>';
			$file_name = $title.'_notes';
			$footer = '<hr>International Academy of Classical Homeopathy';
			$pdf = new PDF();
