...
class UserManager {
    ...
    private static function change_active_state($user_id, $active) {
        if (strval(intval($user_id)) != $user_id) { return false; }
   ...
   public static function get_picture_user($user_id, $picture_file, $height, $size_picture = USER_IMAGE_SIZE_MEDIUM , $style = '') {
    ...
    public static function get_all_user_tags($tag, $field_id = 0, $from = 0, $number_of_items = 10) {
        ...
        if (isset ($keyword)) {
                ...
                $sql .= " WHERE (u.firstname LIKE '%".$keyword."%' OR u.lastname LIKE '%".$keyword."%' OR concat(u.firstname,' ',u.lastname) LIKE '%".$keyword."%' OR concat(u.lastname,' ',u.firstname) LIKE '%".$keyword."%' )";
            }
        $keyword_active = true;
        //only active users
        if ($keyword_active) {
